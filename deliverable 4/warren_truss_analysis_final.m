...
for i = 1:1:loop_length
    ...
    while(list_different) % iterations loop
        ...
        for j = 1:length(data_list)
            ...
            [member_mass, type, max_support, normal_stress, delta_L, normal_strain] = calculate_weight(force, len);
            data_list(j, 4) = {type};
            data_list(j, 5) = {max_support};
            data_list(j, 6) = {double(member_mass)};
            data_list(j, 7) = {double(normal_stress)};
            data_list(j, 8) = {double(delta_L)};
            data_list(j, 9) = {double(normal_strain)};
